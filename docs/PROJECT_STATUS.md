...
**Updated:** 2026-06-09 | **Version:** v1.72.66 | **Completion:** ~99%
...
As of v1.72.63–v1.72.66: Stowage plan detail page visual and UX refinements. (1) VesselProfile SVG capacity progress bars overhauled (v1.72.63–v1.72.64) — bars now rendered last in each compartment <g> so they appear above the footer strip background (SVG painter's-model fix); opacity dilution eliminated; height set to 3 px solid with rounded ends (rx=1); track fill #1e293b; fill colors green #22c55e / amber #fbbf24 / red #f87171 matching ≤75% / ≤90% / >90% thresholds; cargo fill bar opacity raised from 0.35 to 0.65 for richer color. POL port text in the footer strip corrected from low-contrast #60a5fa (blue) to high-contrast #e2e8f0 (light) when ports are present, muted slate (#475569) when empty. (2) Three-way layout transition orchestration (v1.72.65) — all transition durations unified to 300 ms cubic-bezier(0.4, 0, 0.2, 1); sidebar collapse dispatch made synchronous (10 ms setTimeout removed); slideInRight / slideOutRight @keyframes changed from full-width translateX(100%) drawer motion to subtle translateX(32px) professional fade; .compartmentDetailPanelShifted gains transition on right and width; --transition-normal and --transition-slow tokens upgraded to Material Design cubic-bezier easing. (3) Panel open animation slowed for a smoother feel (v1.72.66) — slideInRight duration and compartmentDetailPanel animation/transition durations increased from 300 ms to 480 ms with cubic-bezier(0.22, 1, 0.36, 1) (expo-out spring); slideOutRight close animation kept at 180 ms for snappy dismissal.
