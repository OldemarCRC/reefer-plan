...
export async function autoGenerateDraftPlans() {
  try {
    ...
    const details: { voyageNumber: string; result: string; conflictCount: number; planNumber?: string }[] = [];
    ...
    for (const voyage of voyages as any[]) {
      ...
      if (existingPlan) {
        skipped++;
        details.push({ voyageNumber: voyage.voyageNumber, result: `Plan ${(existingPlan as any).planNumber} already exists`, conflictCount: 0 });
        continue;
      }

      // Fetch CONFIRMED + PARTIAL bookings
      ...
      if (bookings.length === 0) {
        skipped++;
        details.push({ voyageNumber: voyage.voyageNumber, result: 'No confirmed bookings', conflictCount: 0 });
        continue;
      }
      ...
      if (!vessel) {
        skipped++;
        details.push({ voyageNumber: voyage.voyageNumber, result: 'Vessel not found', conflictCount: 0 });
        continue;
      }

      try {
        // Check for prior completed plan on same vessel+service (for INHERITED zone temps)
        ...
        // Map output to plan document fields
        ...
        await StowagePlanModel.create({
          ...
          status:      hasHardConflict ? 'ESTIMATED' : 'DRAFT',
          cargoPositions,
          coolingSectionStatus,
        ...
        details.push({
          ...
          result: 'created',
          planNumber,
          conflictCount: engineOutput.conflicts.filter(c =>
        ...
        details.push({ voyageNumber: voyage.voyageNumber, result: `Engine error: ${msg}`, conflictCount: 0 });
      }
    }
