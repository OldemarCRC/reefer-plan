    ...
    for (const voyage of voyages as any[]) {
      ...
      if (voyage.serviceId) {
        ...
        for (const contract of activeContracts as any[]) {
          ...
          if (coveredContractIds.has(contractId)) continue;
          ...
          const tempRange = getTempRange(contract.cargoType ?? '');
          contractEstimates.push({
            bookingId:     `CONTRACT-ESTIMATE-${contractId}`,
            cargoType:     contract.cargoType ?? 'OTHER_CHILLED',
            tempMin:       tempRange.min,
            tempMax:       tempRange.max,
            pallets,
            polSequence:   polSeq,
      ...
      try {
        ...
        // Build combined booking metadata for cargoType lookup
