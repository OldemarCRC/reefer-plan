...
function buildPortSchedule(service: ServiceData): PortScheduleEntry[] {
  const today = new Date();
  return (service.portRotation ?? [])
    .slice()
...
export default function NewVoyageWizard({ initialServices }: { initialServices: ServiceData[] }) {
  const router = useRouter();

  // If exactly one service is visible, pre-select it and start at Step 2
  const preselected = initialServices.length === 1 ? initialServices[0] : null;

  const [step, setStep] = useState<Step>(preselected ? 2 : 1);
  const [vessels, setVessels] = useState<VesselData[]>([]);
  const [loading, setLoading] = useState(true);

  const [selectedService, setSelectedService] = useState<ServiceData | null>(preselected);
  ...
  const [portSchedule, setPortSchedule] = useState<PortScheduleEntry[]>(
    preselected ? buildPortSchedule(preselected) : []
  );
  ...
  const handleServiceSelect = (service: ServiceData) => {
    ...
    setPortSchedule(buildPortSchedule(service));
  };
